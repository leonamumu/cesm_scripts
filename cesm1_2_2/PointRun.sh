...
if [ ! -f "domain.ocn_noocean.nc" ]; then
	cp $CCSMROOT/tools/mapping/gen_domain_files/domain.ocn.domain.ocn_noocean.nc.${CDATE}.nc \
...
cd $CCSMROOT/scripts
export CASE=${GRIDNAME}
rm -rf ../SinglePointExp/$CASE
rm -rf $RUNDIR/$CASE
rm -rf $DOUTDIR/$CASE
./create_newcase -case ../SinglePointExp/$CASE -res CLM_USRDAT -compset I1PTCLM45 -mach lxfpc
cd ../SinglePointExp/$CASE
