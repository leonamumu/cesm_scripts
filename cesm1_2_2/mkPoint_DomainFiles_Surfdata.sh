...
if [[ $? -eq 0 ]]; then
	...
	sed -i -e 's|LIB_NETCDF=$(NETCDF)/lib|export LIB_NETCDF=$(NETCDF)/lib|' Makefile.common
	sed -i -e 's|LIB_NETCDF=$(NETCDF)/include|export LIB_NETCDF=$(NETCDF)/include|' Makefile.common
fi
